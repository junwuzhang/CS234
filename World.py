...
master = Tk()
master.wm_title("Reinforcement Learning for Path Planning | Junwu Zhang & Yueqi Wang | CS 234 Winter 2019")
...
if not result:
    ...
    Width = global_w / x
    text_offset = Width / 4
    triangle_size = Width * 0.004
    
    path = os.getcwd() + "/images/"
    ghost_pic = PhotoImage(file=path+'ghost.png')
    end_pic = PhotoImage(file=path+'cry.png')
    haunted_pic = PhotoImage(file=path+'haunted_house.png')
    start_pic = PhotoImage(file=path+'ninja.png')

    board = Canvas(master, width=global_w, height=global_h)
    start_count = 0
    goal_count = 0
    ...
    def create_item(event):
        ...
        if item_grid[y][x] == 0:
            if var.get() == "Ghosts":
                item_grid[y][x] = board.create_rectangle(x*Width, y*Width, (x+1)*Width, (y+1)*Width, fill="black", width=1)
                # item_grid[y][x] = board.create_image((x + 0.5)*Width, (y + 0.5)*Width, image=ghost_pic)
                grid[y][x] = 1
            elif var.get() == "Start":
                item_grid[y][x] = board.create_image((x + 0.5)*Width, (y + 0.5)*Width, image=start_pic)
                ...
                item_grid[y][x] = board.create_image((x + 0.5)*Width, (y + 0.5)*Width, image=end_pic)
                grid[y][x] = 3
                goal_count+=1
            elif var.get() == "Haunted House":
                item_grid[y][x] = board.create_image((x + 0.5)*Width, (y + 0.5)*Width, image=haunted_pic)
                grid[y][x] = 4
